...
class ProductsDetails extends React.Component {
  ...
  getColor() {
    return (
      ...
      this.props.ProductState.color.map(color => ({ value: color }))
    );
  }
  ...
  getSize() {
    return (
      this.props.ProductState &&
      this.props.ProductState.size.map(color => ({ value: color }))
    );
  }
        ...
        <Header style={styles.HeaderStyle}>
          ...
          <Body style={{ alignItems: "flex-end" }}>
            <Text style={{ fontSize: 18, fontWeight: "bold", color: "#fff" }}>
              {this.props.ProductState && this.props.ProductState.name}
            </Text>
          </Body>
        ...
        <View style={{ flex: 4 }}>
          ...
          <View style={styles.detailsContainer}>
            <ScrollView>
              ...
              >
                <View>
                  <Text style={styles.text_element_id}>
                    {this.props.ProductState && this.props.ProductState.title}
                  </Text>
                </View>
                ...
                <View>
                  <Rating
                    // fractions={1}
                    startingValue={
                      this.props.ProductState && this.props.ProductState.rating
                    }
                    readonly
                    imageSize={20}
              ...
              <Text style={styles.text_element_item}>
                Model :{" "}
                {this.props.ProductState && this.props.ProductState.model}
              </Text>

              <Text style={styles.text_element_item}>
                Details :{" "}
                {this.props.ProductState && this.props.ProductState.details}
              </Text>

              <Text style={styles.text_element_item}>
                Color :{" "}
                {this.props.ProductState &&
                  this.props.ProductState.color.join(", ")}
              </Text>

              <Text style={styles.text_element_item}>
                Stock :{" "}
                {this.props.ProductState &&
                  this.props.ProductState.avilableStatus}
              </Text>

              <Text style={styles.text_element_item}>
                Discount :{" "}
                {this.props.ProductState && this.props.ProductState.discount}
              </Text>
              ...
              >
                <View>
                  <Text style={styles.text_element_size}>
                    Size :{" "}
                    {this.props.ProductState &&
                      this.props.ProductState.size.join(", ")}
                  </Text>

                  <Text style={styles.text_element_price}>
                    Price :{" "}
                    {this.props.ProductState && this.props.ProductState.price}
                  </Text>
                </View>
              ...
              <View style={{ alignItems: "center", height: 65 }}>
                {/* <Text>Hello</Text> */}
                <Dropdown
                  containerStyle={{ width: 170, height: 60, marginTop: -25 }}
                  baseColor="#ff6f00"
...
//   const actions = {
//     addToCart: (list: []) => addProductToCart(list)
//   };
...
function mapDispatchToProps(dispatch) {
  const actions = {
    //If its error 'addProductToCart' is not defined erase it and type start and select from suggesion
    // addToCart: (index: string) => addProductToCart(index)
    // addToCart: (list: []) => addProductToCart(list)
    addToCart: (index: string) => addProductToCart(index)
  };
  return bindActionCreators(actions, dispatch);
